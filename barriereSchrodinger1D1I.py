...
for i in range(Nx):
    if 0 <= x[i] <= a_barriere:
        V[i] = V0
        # on crée potentiel 0 partout, sauf entre x=0 et x=a_barriere où il vaut V0



psi = np.exp(-((x - x0)**2) / 4) * np.exp(1j * k0 * x)
# on initialise le paquet d'ondes gaussien centré en x0, avec une vitesse k0
...
for it in range(nt - 1):
    for ix in range(1, Nx - 1):
        ...
        # on avance Ψ d'un petit pas de temps dt
